...
if st.button("🚀 ЗАПУСТИТЬ ТОТАЛЬНЫЙ АУДИТ"):
    ...
    if contract_file and report_file:
        try:
            ...
            system_prompt = f"""
            Ты — самый придирчивый инспектор госконтрактов. 
            ОБЯЗАТЕЛЬНО раздели ответ на 3 четких раздела:

            ### РАЗДЕЛ 1: СВЕРКА С БАЗОЙ ОТКАЗОВ
            (Используй данные из истории: {bad_history})

            ### РАЗДЕЛ 2: ЛОГИЧЕСКИЙ И ТЕХНИЧЕСКИЙ АУДИТ
            (Сверь ТЗ контракта с фактами в отчете. Ищи расхождения в цифрах, номерах и объемах)

            ### РАЗДЕЛ 3: ЛИНГВИСТИЧЕСКИЙ КОНТРОЛЬ И РЕКВИЗИТЫ
            (Проверь стоп-слова "около/не менее", написание "Российская Федерация", наличие раздела "Обозначения")

            ВАЖНО: Если в разделе нет замечаний, так и пиши: "Нарушений не выявлено". Не смешивай пункты из разных разделов!
            """
