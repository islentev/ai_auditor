...
if st.button("🚀 ЗАПУСТИТЬ ТОТАЛЬНЫЙ АУДИТ"):
    if contract_file and report_file:
        try:
            ...
            ### СЛОЙ 1: БАЗА ПРОШЛЫХ ОТКАЗОВ
            {bad_history}
            
            ### СЛОЙ 2: ТЕХНИЧЕСКОЕ ЗАДАНИЕ
            Сверь каждый пункт ТЗ с фактами в Отчете.
            
            ### СЛОЙ 3: ЛИНГВИСТИКА И РЕКВИЗИТЫ
            Проверь ИНК, печати, стоп-слова ("около", "не менее"), написание "Российская Федерация".

            ИНСТРУКЦИЯ: Будь максимально придирчивым. Пиши четко: "Нарушение", "Основание", "Риск".
            """
